    ...
    stages {
        ...
        stage('Deploy to develop Server') {
            ...
            steps {
                ...
                sshagent(['lacrosse']) {
                    sh '''ssh lacrosse@182.75.105.186 "cd /home/lacrosse && sh deploy-dev-jobs.sh"'''
                }
            }
            post {
                failure {
                    emailext subject: "Dev Deployment Failed",
                              body: """This is an automated notification regarding the recent deployment of lacrosse in the Dev environment.<br>
                                    ...
                                    DevOps Team <br>
                                    Debut Infotech""",
                              to: "lacrosse-backend@debutinfotech.com",
        ...
        stage('Deploy to QA Server') {
            ...
            steps {
                sshagent(['lacrosse']) {
                    script {
                        def qaDeploymentStatus = sh(script: 'ssh lacrosse@182.75.105.186 "cd /home/lacrosse && sh deploy-qa-jobs.sh"', returnStatus: true)
        ...
        stage('Deploy to UAT Server') {
            ...
            steps {
                sshagent(['ubuntu']) {
                    script {
                        def uatDeploymentStatus = sh(script: 'ssh -o StrictHostKeyChecking=no ubuntu@52.3.34.103 "cd /var/www && sh deploy-uat-jobs.sh"', returnStatus: true)
